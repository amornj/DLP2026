...
const tabs = [
  { id: 'prevent-ascvd', label: 'PREVENT ASCVD' },
  { id: 'ldl-goals', label: 'LDL Goal Helper' },
  { id: 'tg-risk', label: 'TG Severity' },
]
...
export default function CalculatorPage() {
  ...
  const [nonHdl, setNonHdl] = useState('160')
  const [tg, setTg] = useState('320')
  ...
  const goal = useMemo(() => {
    const map: Record<string, { ldl: number; nonHdl: number; note: string }> = {
      borderline: { ldl: 100, nonHdl: 130, note: 'Typical goal when statin is chosen in borderline risk primary prevention.' },
      intermediate: { ldl: 100, nonHdl: 130, note: 'Typical goal in intermediate-risk primary prevention.' },
      high: { ldl: 70, nonHdl: 100, note: 'Common target framework for higher-risk primary prevention.' },
      cac100: { ldl: 70, nonHdl: 100, note: 'Guideline-aligned target for CAC 100–299 or ≥75th percentile.' },
      ascvd: { ldl: 55, nonHdl: 85, note: 'Reasonable high-intensity target framework for very high-risk secondary prevention.' },
      severe: { ldl: 70, nonHdl: 100, note: 'Severe hypercholesterolemia often needs early combination therapy to this range.' },
    }

    const chosen = map[scenario]
    const ldlNum = Number(ldl) || 0
    const nonHdlNum = Number(nonHdl) || 0

    return {
      ...chosen,
      ldlGap: Math.max(ldlNum - chosen.ldl, 0),
      nonHdlGap: Math.max(nonHdlNum - chosen.nonHdl, 0),
    }
  }, [scenario, ldl, nonHdl])

  const tgResult = useMemo(() => {
    const value = Number(tg) || 0
    if (value >= 1000) return { label: 'Very severe', color: 'badge-red', note: 'Urgent pancreatitis-prevention pathway.' }
    if (value >= 500) return { label: 'Severe', color: 'badge-yellow', note: 'Pancreatitis risk is now a direct management issue.' }
    if (value >= 150) return { label: 'Mild–moderate', color: 'badge-blue', note: 'Focus on lifestyle, metabolic drivers, and ASCVD risk strategy.' }
    return { label: 'Normal / near normal', color: 'badge-green', note: 'Maintain healthy lifestyle and treat overall risk context.' }
  }, [tg])
  ...
  return (
    <div>
      <h1 className="text-2xl md:text-3xl font-bold text-primary mb-1">Calculators</h1>
      <p className="text-sm text-gray-500 mb-4">Quick bedside helpers based on guideline thresholds</p>
      ...
      <div className="flex flex-wrap gap-1 mb-6">
        {tabs.map((tab) => (
          <button
            ...
            className={`tab-btn ${calculatorTab === tab.id ? 'tab-btn-active' : 'tab-btn-inactive'}`}
          >
            {tab.label}
          </button>
        ))}
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 border-t">
                  <div>
                    ...
                    <input
                      ...
                      step="0.1"
                      value={prevent.hba1c ?? ''}
                      onChange={(e) => updatePrevent('hba1c', e.target.value === '' ? undefined : Number(e.target.value))}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      step="0.1"
                      value={prevent.uacr ?? ''}
                      onChange={(e) => updatePrevent('uacr', e.target.value === '' ? undefined : Number(e.target.value))}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      maxLength={5}
                      value={prevent.zipCode ?? ''}
                      onChange={(e) => updatePrevent('zipCode', e.target.value === '' ? undefined : e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      max={10}
                      value={prevent.sdi ?? ''}
                      onChange={(e) => updatePrevent('sdi', e.target.value === '' ? undefined : Number(e.target.value))}
                    />
                  </div>
                </div>
              )}
      ...
      {calculatorTab === 'ldl-goals' && (
        <div className="card space-y-4">
          <h2 className="card-header">LDL / non–HDL Goal Helper</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700">Scenario</label>
              <select className="input-field mt-1" value={scenario} onChange={(e) => setScenario(e.target.value)}>
                <option value="borderline">Borderline primary prevention</option>
                <option value="intermediate">Intermediate primary prevention</option>
                <option value="high">High-risk primary prevention</option>
                <option value="cac100">CAC 100–299 or ≥75th percentile</option>
                <option value="severe">Severe hypercholesterolemia / FH phenotype</option>
                <option value="ascvd">Very high-risk ASCVD</option>
              </select>
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Current LDL-C (mg/dL)</label>
              <input className="input-field mt-1" value={ldl} onChange={(e) => setLdl(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Current non–HDL-C (mg/dL)</label>
              <input className="input-field mt-1" value={nonHdl} onChange={(e) => setNonHdl(e.target.value)} />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-xs text-blue-800 font-semibold uppercase">Target LDL-C</p>
              <p className="text-3xl font-bold text-blue-900 mt-1">&lt; {goal.ldl}</p>
              <p className="text-sm text-gray-600 mt-2">Gap to target: {goal.ldlGap} mg/dL</p>
            </div>
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <p className="text-xs text-purple-800 font-semibold uppercase">Target non–HDL-C</p>
              <p className="text-3xl font-bold text-purple-900 mt-1">&lt; {goal.nonHdl}</p>
              <p className="text-sm text-gray-600 mt-2">Gap to target: {goal.nonHdlGap} mg/dL</p>
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-700 font-semibold uppercase">Interpretation</p>
              <p className="text-sm text-gray-700 mt-2">{goal.note}</p>
            </div>
          </div>
        </div>
      )}

      {calculatorTab === 'tg-risk' && (
        <div className="card space-y-4">
          <h2 className="card-header">Triglyceride Severity Categorizer</h2>
          <div className="max-w-md">
            <label className="text-sm font-medium text-gray-700">Fasting TG (mg/dL)</label>
            <input className="input-field mt-1" value={tg} onChange={(e) => setTg(e.target.value)} />
          </div>
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <p className="text-sm text-gray-500">Category</p>
            <div className="mt-2">
              <span className={tgResult.color}>{tgResult.label}</span>
            </div>
            <p className="text-sm text-gray-700 mt-3">{tgResult.note}</p>
          </div>
        </div>
      )}
    </div>
